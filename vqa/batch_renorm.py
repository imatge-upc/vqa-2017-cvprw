...
class BatchRenormalization(Layer):
    ...
    def call(self, x, mask=None):
        if self.mode == 0 or self.mode == 2:
            ...
            mean_batch, var_batch = tf.nn.moments(x, reduction_axes, shift=None, keep_dims=False)
    ...
    def get_config(self):
        config = {'epsilon': self.epsilon,
                  'mode': self.mode,
                  'axis': self.axis,
                  'gamma_regularizer': self.serialize(self.gamma_regularizer),
                  'beta_regularizer': self.serialize(self.beta_regularizer),
    ...
    def serialize_keras_object(self, instance):
        if instance is None:
            return None
    ...
    def serialize(self, regularizer):
        return self.serialize_keras_object(regularizer)
